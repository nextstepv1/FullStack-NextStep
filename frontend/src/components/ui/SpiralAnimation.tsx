...
class AnimationController {
    ...
    private createStars() {
        this.stars = []
        // Use custom PRNG to get identical seeded star positions
        const lcgRandom = () => {
            let seed = 1234
            return () => {
...
class Star {
    ...
    render(p: number, controller: AnimationController) {
        ...
        if (q > 0) {
            ...
            // Access public readonly fields safely without bracket notation
            const cameraZ = controller.cameraZ
            const viewZoom = controller.viewZoom
...
export function SpiralAnimation({ bgColor = 'white', particleColor = '#001734' }: SpiralAnimationProps) {
    ...
    useEffect(() => {
        ...
        animationRef.current = new AnimationController(ctx, size, bgColor, particleColor)
    ...
    }, [dimensions, bgColor, particleColor])
