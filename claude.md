...
> **Last updated: 2026-06-01** — After forgot-password OTP flow implementation.
...
└── frontend/                        ← React + Vite + TypeScript
    ...
    └── src/
        ...
        │   ├── auth/
        │   │   ├── AuthPage.tsx     ← ✅⭐ Auth hub TERPUSAT (Login + Register + Forgot PW dalam 1 file)
        │   │   │                        Route /login, /register, /forgot-password
        │   │   │                        Fitur: vertical slide antar mode, OTP 6-digit, strength meter
        │   │   │                        Bilingual ID/EN, mobile responsive 38vh+62vh split
        │   │   ├── LoginPage.tsx    ← ⚠️ Legacy — tidak dipakai routing (bisa dihapus)
        │   │   └── RegisterPage.tsx ← ⚠️ Legacy — tidak dipakai routing (bisa dihapus)
        │   ├── cv/
...
| `/login` | `AuthPage` (mode login) | No (redirect jika sudah login) | ✅ Done |
| `/register` | `AuthPage` (mode register) | No | ✅ Done |
| `/forgot-password` | `AuthPage` (mode forgot password) | No | ✅ Done |
...
> Lupa password juga masih simulasi (delay + state lokal) — backend perlu mengimplementasi 3 endpoint OTP.
...
POST   /api/auth/forgot-password   → Terima email, kirim OTP 6-digit via email
POST   /api/auth/verify-otp        → Verifikasi OTP, return reset_token (JWT/UUID short-lived)
POST   /api/auth/reset-password    → Ganti password pakai reset_token + new_password
```

**Kontrak payload lupa password (dipanggil dari `AuthPage.tsx`):**
```json
// Step 1 — POST /api/auth/forgot-password
{ "email": "user@email.com" }
// Response: { "success": true, "message": "OTP dikirim ke email Anda." }

// Step 2 — POST /api/auth/verify-otp
{ "email": "user@email.com", "otp": "123456" }
// Response: { "success": true, "data": { "reset_token": "<token>" } }
...
> **OTP Rules:**
> - 6 digit numerik, dikirim via email (nodemailer)
> - OTP expired setelah **5 menit**
> - `reset_token` expired setelah **15 menit**, single-use
> - Jika email tidak terdaftar, tetap return success (security best practice — jangan bocorkan info user)
...
| Forgot PW belum ke backend | `AuthPage.tsx` (cari `TODO (Backend)`) | 🔴 Blocker | 3 endpoint OTP perlu dibuat backend |
...
| `LoginPage.tsx` & `RegisterPage.tsx` legacy | `frontend/src/pages/auth/` | 🟢 Low | Tidak dipakai routing — bisa dihapus saat cleanup |
